...
| `updatePart(messageId, partId, patch)`                           | Shallow-merge `patch` into the matching part (e.g. grow `text`, flip `status`). Keyed by `id`, so stateful elements survive. Validated like `tryUpdatePart`; since it returns nothing, a rejected patch is dropped and reported to the console. |
| `tryUpdatePart(messageId, partId, patch)`                        | The same update, returning `{ ok, reason? }` so the caller can handle a rejection instead.                                                                                                                                                      |
